...
public class WaarpFtp4jClient {
	 ...
	 * @param timeout
	 */
	public WaarpFtp4jClient(String server, int port, String user, String pwd, String acct, boolean isPassive, int ssl,
	...
	public boolean makeDir(String newDir) {
		...
		try {
			...
			System.err.println("T0 directory is already exist");
			return false;
		} catch (IllegalStateException e) {
			System.err.println("T0 directory is already exist");
			return false;
			...
			if (getStoreOrAppend > 0) {
				...
				try {
					if (getStoreOrAppend == 1) {
						this.ftpClient.upload(from, new DataTimeOutListener(ftpClient, timeout, "STOR", local));
					} else {
						// append
						this.ftpClient.append(from, new DataTimeOutListener(ftpClient, timeout, "APPE", local));
					}
					result = null;
				} catch (IllegalStateException e) {
					logger.error(result, e);
				...
				if (local == null) {
					...
					try {
						this.ftpClient.download(remote, to, new DataTimeOutListener(ftpClient, timeout, "RETR", local));
						result = null;
					} catch (IllegalStateException e) {
						logger.error(result, e);
						return false;
