...
/**
 * FTP Thread used to check multiple FTP clients in parallel with the test
 * scenario
 * 
 * @author frederic
 * 
...
public class FtpClientThread implements Runnable {
	 ...
	 * @param delay
	 */
	public FtpClientThread(String id, String server, int port, String username, String passwd, String account,
			String localFilename, int nb, int type, int delay, int isSsl) {
		this.id = id;
		this.server = server;
		this.port = port;
	...
	public void run() {
		Ftp4JClientTransactionTest client = new Ftp4JClientTransactionTest(this.server, this.port, this.username,
				this.passwd, this.account, this.isSsl);
		// Thread.yield();
			...
			if (type <= 0) {
				...
				if (type <= -10) {
					for (int i = 0; i < this.numberIteration; i++) {
						...
						if (!client.transferFile(this.localFilename, this.remoteFilename, true)) {
							System.err.println((new Date()) + " Cant store file passive mode " + this.id);
							FtpClient.numberKO.incrementAndGet();
							return;
						} else {
							FtpClient.numberOK.incrementAndGet();
					...
					for (int i = 0; i < this.numberIteration; i++) {
						...
						if (!client.transferFile(localfile, this.remoteFilename, false)) {
							System.err.println((new Date()) + " Cant retrieve file passive mode " + this.id);
							FtpClient.numberKO.incrementAndGet();
							return;
						} else {
							FtpClient.numberOK.incrementAndGet();
			...
			if (type >= 0) {
				...
				if (type >= 10) {
					for (int i = 0; i < this.numberIteration; i++) {
						// System.err.println(id+" transfer store "+i);
						if (!client.transferFile(this.localFilename, this.remoteFilename, true)) {
							System.err.println((new Date()) + " Cant store file active mode " + this.id);
							FtpClient.numberKO.incrementAndGet();
							return;
						} else {
							FtpClient.numberOK.incrementAndGet();
					...
					for (int i = 0; i < this.numberIteration; i++) {
						if (!client.transferFile(this.localFilename, this.remoteFilename, false)) {
							System.err.println("File downloading is failed");
						} else {
							FtpClient.numberOK.incrementAndGet();
